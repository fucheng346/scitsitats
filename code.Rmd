...
baseline_value <- mean(df_train$charges.t)

df_train$baseline_pred <- rep(baseline_value, nrow(df_train))

base_rmse <- sqrt(mean((df_train$charges.t - df_train$baseline_pred)^2))
base_mae  <- mean(abs(df_train$charges.t - df_train$baseline_pred))
...
```{r placeholder Model, echo=TRUE}

mod.all <- lm(charges.t ~ age.t + sex + bmi + children + smoker + region, df_train)
summary(mod.all)
...
step(mod.all, direction = "backward", trace = 1)
